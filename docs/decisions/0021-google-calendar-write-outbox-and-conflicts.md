...
**Status:** Accepted as the provider-write **safety** boundary. Its human
conflict-resolution policy is superseded and withdrawn — see ADR 0022.

**Date:** 2026-08-30 · **Amended:** 2026-09-01

> **Amendment (2026-09-01, owner-directed after real acceptance testing).**
> This ADR is retained for the half that proved correct and withdrawn for the
> half that did not.
>
> **Retained, unchanged and binding:** the Rust/Python/React authority split,
> canonical-intent-before-dispatch, the durable outbox, exact non-wildcard
> `If-Match`, no silent last-write-wins, no timestamp authority, the narrow
> changed-field provider body, deterministic create identity, bounded
> restart-safe retry, and the sanitized audit boundary. These are the
> provider-write safety layer and the rebuild keeps them.
>
> **Superseded and withdrawn:** the rule that a stale precondition or a newly
> observed provider version produces a conflict a person must resolve, and the
> `Keep Google version` / `Apply my Ion changes` / `Review differences` surface
> that rule required. In real use it turned every ordinary edit into a review
> task. Ordinary ETag drift is now re-read and rebased automatically inside the
> bounded attempt budget, and the version chooser is not reimplemented in any
> form. Conditions that genuinely need a person are a closed set of specific
> named recovery states.
>
> **Human authorization and provider-write safety are separate layers.**
> Withdrawing a confirmation changes the first and never weakens the second.
> The current interaction contract is
> [Calendar interaction behavior](../CALENDAR_BEHAVIOR.md); the rebuild that
> implements it is [ADR 0022](0022-phase-2c-controlled-rebuild.md).
...
truthfully, stops as one of a closed set of specifically named recovery
conditions, each offering only actions that are truthful for that condition.
**The `Keep Google version` / `Apply my Ion changes` / `Review differences`
resolution surface this paragraph originally specified is withdrawn** and is
not reimplemented; see ADR 0022 and
[Calendar interaction behavior](../CALENDAR_BEHAVIOR.md). Ion-only metadata
never participates in a provider conflict.
