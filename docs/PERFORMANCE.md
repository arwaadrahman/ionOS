...
Phase 2C-5 Calendar create/edit/delete/recurrence adds no polling timer or background loop. Each
...
bounded same-event lookup, including delete ambiguity. An occurrence mutation
adds one bounded master lookup and one `events.instances` result page capped at
two candidates before exact-instance dispatch. Retry timestamps remain durable
