...
### Rows from the superseded policy

Conflict rows created by the old "every mismatch needs review" policy describe
ordinary drift. Recovery re-arms them against confirmed authority automatically,
so upgrading into the new behavior does not leave the owner holding a permanent
review task. Their original conflict audit is preserved; a row without enough
durable evidence to rebase safely is left explicit rather than guessed at.
...
### Apply my Ion changes is not ordinary Calendar UX

Keep Google / Review differences / **Apply my Ion changes** are a last-resort
surface for drift that survived the entire automatic rebase budget. They must
never appear for ordinary ETag drift, pending-write progression, provider
confirmation, retry, post-save continuation, or post-sync continuation.

**Apply my Ion changes is never required for normal Calendar operation.** If a
user can reach it during an ordinary uncontested edit, the state transition that
let them is the bug.
...
- [Phase 2C gate](phases/PHASE_2C.md) — accepted recurrence write semantics.
- [ADR 0021](decisions/0021-google-calendar-write-outbox-and-conflicts.md) —
  outbox, ETag, and conflict model.
